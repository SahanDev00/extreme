...
const CartIndex = () => {
  ...
  const { cartItems, updateQuantity, calculateTotal, removeFromCart } =
    useCart();
  ...
  const generatePDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text("Extreme Quotation", 14, 28);
  ...
  return (
    <div className="w-full pt-[50px] min-h-screen">
      ...
      {cartItems.length === 0 ? (
        <p className="text-black/70 ml-10 md:ml-40 text-sm md:text-xl font-overpass">
          Your cart is empty.
        </p>
      ) : (
        <div>
          <ul className="w-[90%] xl:w-[85%] mx-auto">
            {cartItems.map((item) => (
              <li
                key={item.itemID}
                className="flex justify-between mb-4 py-4 border-b"
              >
                <div className="flex items-center w-[85%]">
                  ...
                  <div className="md:flex items-center justify-between w-full">
                    <div>
                      <h2 className="text-sm sm:text-xl text-blue-700 font-semibold font-overpass">
                        {item.itemName}
                      </h2>
                      <p className="text-black/70 font-overpass text-xs sm:text-[16px]">
                    ...
                    <div className="flex items-center mt-2 md:mt-0">
                      ...
                      <span className="mx-2 text-black/70 font-overpass text-sm sm:text-[16px]">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() => updateQuantity(item.itemID, 1)}
                ...
                <div className="text-right flex justify-center items-center ml-3">
                  <p className="text-sm sm:text-lg text-black/70 font-semibold font-overpass">
                    Rs.{" "}
                    {Number(formatPrice(item.retailPrice)).toLocaleString(
                      "en-LK"
                    )}
                  </p>
                </div>
          ...
          <div className="w-[90%] xl:w-[85%] mx-auto flex items-center justify-end mt-4">
            <p className="text-lg sm:text-xl text-black/70 font-semibold font-overpass">
              Total: Rs.{" "}
              {Number(formatPrice(calculateTotal())).toLocaleString("en-LK")}
            </p>
          </div>
          <div className="w-[90%] xl:w-[85%] mx-auto pb-5 gap-4 flex items-center justify-end mt-4">
            <button
              onClick={handleCheckout}
              className="bg-green-500 text-white py-2 px-4 rounded font-overpass"
            >
              Check Out
            </button>
            <button
              onClick={generatePDF}
              className="bg-amber-400 text-white py-2 px-4 rounded font-overpass"
            >
              Download Quotation
            </button>
          </div>
