...
const MyOrderView = ({ viewOrder, onClose }) => {
    ...
    useEffect(() => {
        const fetchOrderDetails = async () => {
            ...
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/OrderItem?OrderID=${viewOrder.orderID}`, {
                    headers: {
                        'APIKey': apiKey
                    }
                });
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
        <div className='w-[90%] md:w-[70%] bg-white p-5 relative'>
            <h1 className='text-2xl text-center my-8 font-bold text-black/70 font-karla'>Order Details : {viewOrder.orderID}</h1>
