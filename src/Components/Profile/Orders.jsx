...
const Orders = () => {
  ...
  if (orders.length === 0) {
    return <div className='bg-white h-[85vh] w-full mx-auto pt-[50px] font-karla'>
      <h1 className='font-semibold mt-5 text-center'>You Don't Have Orders Yet!!!</h1>
    </div>
  }

  return (
    <div className='w-full md:min-h-[700px] lg:min-h-[650px] xl:min-h-[700px] 2xl:min-h-[800px] mx-auto px-4 py-8 mb-5 pt-[50px] bg-white'>
    <div className='w-[85%] mx-auto h-[200px] md:h-[200px] flex flex-col justify-center'>
        <h1 className='text-3xl sm:text-4xl md:text-5xl font-roboto font-semibold md:w-[700px]'>
            Your Orders. <span className='text-gray-500'>Products You Have Chosen.</span>
        </h1>
    </div>
            ...
            {orders.map(order => (
              <tr key={order.orderId}>
                ...
                <td className='px-4 font-karla py-2 border-b border-black/60'>
                    <button onClick={() => {setIsOrderOpen(true); setViewOrder(order);}} className='bg-blue-500 font-karla text-white px-4 py-2 hover:bg-blue-600 duration-200'>
                        View
                    </button>
                </td>
              </tr>
