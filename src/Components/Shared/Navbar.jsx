...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchMacItems = async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/Item?CategoryMainID=CTM_0015`,
        {
          headers: {
  ...
  useEffect(() => {
    const fetchIphoneItems = async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/Item?CategoryMainID=CTM_0014`,
        {
          headers: {
            APIKey: process.env.REACT_APP_API_KEY,
          },
  ...
  useEffect(() => {
    const fetchLaptopItems = async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/Item?CategoryMainID=CTM_0021`,
        {
          headers: {
            APIKey: process.env.REACT_APP_API_KEY,
          },
  ...
  useEffect(() => {
    const fetchComputerItems = async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/Item?CategoryMainID=CTM_0016`,
        {
          headers: {
            APIKey: process.env.REACT_APP_API_KEY,
          },
  ...
  useEffect(() => {
    const fetchMonitorItems = async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/Item?CategoryMainID=CTM_0017`,
        {
          headers: {
            APIKey: process.env.REACT_APP_API_KEY,
          },
  ...
  useEffect(() => {
    const fetchKeyboardMouseItems = async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/Item?CategoryMainID=CTM_0018`,
        {
          headers: {
            APIKey: process.env.REACT_APP_API_KEY,
          },
  ...
  useEffect(() => {
    const fetchAccessoriesItems = async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/Item?CategoryMainID=CTM_0019`,
        {
          headers: {
            APIKey: process.env.REACT_APP_API_KEY,
          },
  ...
  useEffect(() => {
    const fetchCasingItems = async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/Item?CategoryMainID=CTM_0020`,
        {
          headers: {
            APIKey: process.env.REACT_APP_API_KEY,
          },
  ...
  return (
    <div
      className={`w-full h-[50px] duration-500 backdrop-blur-sm fixed z-50 ${
        isActive("/") ? "bg-stone-900/90 hover:bg-stone-900" : "bg-gray-50"
      }`}
    >
      <div className="w-[96%] md:w-full lg:w-[90%] xl:w-[70%] 2xl:w-[55%] mx-auto h-full flex justify-between md:justify-evenly items-center">
        ...
        {navItems.map((nav, index) => (
          <div key={index} className="group h-full flex items-center">
            <Link to={nav.Link}>
              <p
                className={`hidden md:block text-xs font-overpass font-light cursor-pointer p-2 ${
                  isActive("/")
            ...
            <div
              className={`w-screen left-0 top-[50px] h-[0px] absolute group-hover:h-[400px] duration-500 overflow-hidden ${
                isActive("/") ? "bg-stone-900" : "bg-gray-50 shadow-lg"
              }`}
            >
              <div className="md:w-full lg:w-[90%] xl:w-[70%] 2xl:w-[55%] flex h-full mx-auto">
                <div className="flex flex-col justify-center p-4 opacity-0 group-hover:opacity-100 duration-1000">
                  <h1
                    className={`font-overpass text-sm ${
                  ...
                  {nav.subHeading === "All Items" ? (
                    <Link to="all-products">
                      <p
                        className={`text-3xl font-karla font-semibold mb-1 cursor-pointer ${
                          isActive("/")
                            ? "text-gray-200 hover:text-white"
                    ...
                    <p
                      className={`text-3xl font-karla font-semibold mb-1 ${
                  ...
                  {nav.items.map((item, index) => (
                    <Link key={index} to={item.Link}>
                      <p
                        className={`text-3xl font-karla font-semibold cursor-pointer mb-1 line-clamp-1 ${
                          isActive("/")
                            ? "text-gray-200 hover:text-white"
                            : "text-gray-600 hover:text-gray-800"
                        }`}
        ...
        <FaSearch
          ...
          className={`hidden md:block cursor-pointer size-4 ${
            isActive("/")
              ? "text-gray-200 hover:text-white"
        ...
        <div className="relative">
          <MdOutlineShoppingBag
            onClick={() => toggleBag()}
            className={`hidden md:block cursor-pointer size-5 ${
              isActive("/")
                ? "text-gray-200 hover:text-white"
                : "text-gray-500 hover:text-gray-800"
            }`}
          />
          {getTotalItems() > 0 && (
            <p className="absolute -top-2 -right-4 bg-gray-500 text-white rounded-full px-1 text-xs hidden md:block">
              {getTotalItems()}
            </p>
          )}
        </div>
        ...
        <div className="flex gap-6 justify-evenly items-center md:hidden relative">
          ...
          <div className="relative">
            ...
            />
            {getTotalItems() > 0 && (
              <p className="absolute -top-2 -right-4 bg-gray-600 text-white rounded-full px-1 text-xs md:hidden ">
                {getTotalItems()}
              </p>
            )}
          </div>
      ...
      {/* searchbar */}
      <div
        className={`w-screen left-0 top-0 md:top-[50px] absolute overflow-hidden duration-300 ${
          isActive("/") ? "bg-stone-900" : "bg-gray-50 shadow-lg"
        } ${
          !isSearchBar
            ? "h-[0px] opacity-0"
            : "h-screen md:h-[300px] opacity-100"
        }`}
      >
        <IoClose
          onClick={() => toggleSearchBar()}
          className={`md:hidden cursor-pointer size-6 fixed top-3 right-3 duration-300 z-50 ${
            isActive("/")
              ? "text-gray-200 hover:text-white"
              : "text-gray-500 hover:text-gray-800"
          } ${isBag && "hidden"} ${isSearchBar ? "opacity-100 " : "opacity-0"}`}
        />
          ...
          <div className="flex flex-col md:justify-center duration-1000 font-karla">
            <div className="flex items-center h-[70px] gap-4">
              ...
              <input
                ...
                type="text"
                placeholder="Search on Extreme..."
                className={`bg-transparent p-2 w-full md:w-[600px] outline-none text-lg ${
                  isActive("/") ? "text-gray-200" : "text-gray-500"
                }`}
              />
            </div>
            <div className=" flex flex-col justify-center mt-5 duration-1000">
              <h1
                className={`font-overpass text-sm mb-2 ${
                  isActive("/") ? "text-gray-400" : "text-gray-600"
                }`}
              >
                Quick Links
              ...
              <Link to="/laptops" onClick={() => setIsSearchBar(false)}>
                <p
                  className={`text-xl font-karla font-semibold cursor-pointer mb-1 ${
                    isActive("/")
                      ? "text-gray-200 hover:text-white"
                      : "text-gray-600 hover:text-gray-800"
                  }`}
      ...
      <div
        className={`w-screen left-0 top-0 md:top-[50px] absolute overflow-hidden duration-300 ${
          ...
          !isBag ? "h-[0px] opacity-0" : "h-screen md:h-[300px] opacity-100"
        }`}
      >
        <IoClose
          onClick={() => toggleBag()}
          ...
          <div className="flex flex-col md:justify-center duration-1000 font-karla p-12 sm:p-14">
            {!customerId && (
              <div>
                ...
                <p
                  className={`text-xs font-karla font-semibold mb-1 ${
                    isActive("/") ? "text-gray-200" : "text-gray-500"
                  }`}
                >
                  <a
      ...
      {/* mini nav panel */}
      <div
        className={`absolute w-screen left-0 top-0 overflow-hidden duration-300 z-50 ${
          isActive("/") ? "bg-stone-900" : "bg-gray-50 shadow-lg"
        } ${isNavbar ? "h-screen opacity-100" : "h-0 opacity-0"}`}
      >
        <IoClose
          onClick={() => toggleNavbar()}
          className={`text-gray-200 hover:text-white cursor-pointer size-6 fixed top-3 right-3 duration-300 ${
            isSearchBar && "hidden"
          } ${isBag && "hidden"} ${
            isNavbar ? "opacity-100  z-50" : "opacity-0"
          }`}
        />
        <div className="h-full w-full p-12 sm:p-14">
          <Link to="/all-products" onClick={() => setIsNavbar(false)}>
            <p
              className={`text-3xl font-karla font-semibold cursor-pointer mb-3 ${
                isActive("/") ? "text-gray-200" : "text-gray-600"
              }`}
            >
              All Items
