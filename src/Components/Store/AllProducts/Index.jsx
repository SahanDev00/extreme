...
const Index = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/CategoryMain`,
        {
          headers: {
            APIKey: process.env.REACT_APP_API_KEY,
          },
  ...
  const fetchBrandProducts = async (BrandID) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/Item?&BrandID=${BrandID}`,
        {
          headers: {
            APIKey: process.env.REACT_APP_API_KEY,
          },
  ...
  const fetchSubCategories = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/CategorySub`,
        {
          headers: {
            APIKey: process.env.REACT_APP_API_KEY,
          },
    ...
    <div className="w-full min-h-screen pt-[50px]">
      <div className="w-[85%] mx-auto h-[200px] md:h-[200px] flex flex-col justify-center">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-roboto font-semibold md:w-[700px]">
          All Products.{" "}
          <span className="text-gray-500">From Zero to Hero.</span>
        </h1>
      </div>
            ...
            <div onClick={() => fetchBrandProducts(brand.BrandID)} key={index}>
              <div className="flex flex-col items-center justify-center p-2 cursor-pointer">
                <img
                  ...
                  alt={brand.name}
                />
                <p className="mt-2 font-roboto text-sm text-center">
                  {brand.name}
                </p>
              </div>
            </div>
          ...
          <div onClick={() => fetchBrandProducts(brand.BrandID)} key={index}>
            <div className="w-full h-full flex flex-col items-center justify-center cursor-pointer group">
              <img
                ...
                alt={brand.name}
              />
              <p className="mt-2 font-roboto text-sm group-hover:underline">
                {brand.name}
              </p>
            </div>
          </div>
      ...
      <div className="w-[90%] 2xl:w-[85%] mx-auto my-10 md:flex items-center justify-between">
        {brandName ? (
          <h1 className="text-3xl font-roboto font-semibold">
            <span className="text-gray-600">{brandName}</span> Products
          </h1>
        ) : (
          <h1 className="text-3xl font-roboto font-semibold">
            <span className="text-gray-600">All</span> Products
          </h1>
        )}
      </div>
      ...
      <div className="w-[90%] 2xl:w-[85%] mx-auto md:flex justify-between pb-10">
        <div className="w-[90%] mx-auto md:mx-0 md:w-[200px] lg:w-[200px] 2xl:w-[230px] mb-10 md:mb-0">
          <div className="w-full flex items-center justify-between">
            <h1 className="text-gray-700 font-semibold text-lg mb-2 font-karla">
              Categories
            </h1>
            {brandName && (
              <p
                className="cursor-pointer text-red-500 text-xs font-poppins"
                title="Clear filters"
                onClick={() => {
                  fetchProducts();
                  setBrandName("");
          ...
          {categories.map((category) => (
            <div
              key={category.categoryMainID}
              className="w-full mt-2 text-lg text-gray-800 font-light"
            >
              <div
              ...
              {expandedCategory === category.categoryMainID && (
                <ul className="pl-4 mb-2 border-l transition-all duration-300 ease-in-out">
                  {subCategories
                    .filter(
                      (subCategory) =>
                        subCategory.categoryMainID === category.categoryMainID
                    )
                    .map((subCategory) => (
                      <div
                        key={subCategory.categorySubID}
        ...
        <div className="w-full md:w-[70%] lg:w-[80%] mx-auto md:mx-0 grid sm:grid-cols-2 lg:grid-cols-3 gap-6 2xl:gap-8 pb-10">
          {items.length > 0 ? (
            items.map((item, index) => (
              <div
                key={index}
                className="w-full h-[400px] mx-auto lg:h-[450px] flex flex-col items-center justify-center rounded-2xl bg-white shadow hover:shadow-lg hover:scale-[102%] duration-300"
              >
                <img
                  src={
                  ...
                  className="h-[200px] lg:h-[250px] mx-auto object-contain p-2 xl:p-3"
                  alt=""
                />
                <h1
                  className="text-lg xl:text-2xl font-karla font-semibold mb-1 text-center w-[95%] mx-auto line-clamp-2"
                  title={item.itemName}
                >
                  {item.itemName}
                </h1>
                ...
                {item.stockBalance > 0 ? (
                  <div className="flex items-center h-[50px] justify-center gap-5 mt-2">
                    <Link to={`/items/${item.itemID}`}>
                      <button className="px-4 lg:px-6 py-1 lg:py-2 border rounded-full bg-blue-500 text-white hover:bg-blue-500/90 font-karla">
                        Learn More
                      </button>
                    </Link>
                    <BsCartPlus
                  ...
                  <div className="flex items-center h-[50px] justify-center gap-5 mt-2">
                    ...
                    <button className="px-4 lg:px-6 py-1 lg:py-2 rounded-full text-red-500 font-karla cursor-not-allowed">
                      Out of Stock
                    </button>
                  </div>
                )}
