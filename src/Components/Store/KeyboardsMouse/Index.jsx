...
const Index = () => {
  ...
  const fetchSubProducts = async (SubID) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/Item?CategoryMainID=CTM_0018&CategorySubID=${SubID}`, {
        headers: {
          'APIKey' : process.env.REACT_APP_API_KEY
        }
      })
  ...
  const fetchSubCategories = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/CategorySub?CategoryMainID=CTM_0018`, {
        headers: {
          'APIKey' : process.env.REACT_APP_API_KEY
        }
      })
  ...
  const fetchBrandProducts = async (BrandID) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/Item?CategoryMainID=CTM_0018&BrandID=${BrandID}`, {
        headers: {
          'APIKey' : process.env.REACT_APP_API_KEY
        }
      })
    ...
    <div className='w-full min-h-screen pt-[50px]'>
            <div className='w-[85%] mx-auto h-[200px] md:h-[200px] flex flex-col justify-center'>
                <h1 className='text-3xl sm:text-4xl md:text-5xl font-roboto font-semibold md:w-[700px]'>
                    Keyboards and Mouse. <span className='text-gray-500'>From Beauty to Peformance.</span>
                </h1>
            </div>
