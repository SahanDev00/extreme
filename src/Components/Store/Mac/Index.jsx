...
const Index = () => {
  ...
  const fetchSubProducts = async (SubID) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/Item?CategoryMainID=CTM_0015&CategorySubID=${SubID}`, {
        headers: {
          'APIKey' : process.env.REACT_APP_API_KEY
        }
      })
  ...
  const fetchSubCategories = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/CategorySub?CategoryMainID=CTM_0015`, {
        headers: {
          'APIKey' : process.env.REACT_APP_API_KEY
        }
      })
    ...
    <div className='w-full min-h-screen pt-[50px]'>
            <div className='w-[85%] mx-auto h-[200px] md:h-[200px] flex flex-col justify-center'>
                <h1 className='text-3xl sm:text-4xl md:text-5xl font-roboto font-semibold md:w-[700px]'>
                    Mac Books. <span className='text-gray-500'>Fast and Reliable.</span>
                </h1>
            </div>
            
            <div className='w-[90%] 2xl:w-[85%] mx-auto mb-10 md:flex items-center justify-between'>
              <h1 className='text-3xl font-roboto font-semibold'><span className='text-gray-600'>All</span> Mac Books</h1>
